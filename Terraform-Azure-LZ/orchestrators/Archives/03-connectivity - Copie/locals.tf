...
# Itérer sur les clés uniquement (non sensibles)
  ...
  # ---------------------------------------------------------------------------
  # APPLICATION GATEWAY SUBNET (add to custom_subnets_aue)
  # ---------------------------------------------------------------------------
  # Add this to the custom_subnets_aue map:
  appgw_subnet_aue = var.enable_application_gateway ? {
    ApplicationGatewaySubnet = {
      name             = "ApplicationGatewaySubnet"
      address_prefixes = [var.application_gateway_subnet_address_prefix]
    }
  ...
  all_custom_subnets_aue = merge(local.custom_subnets_aue, local.appgw_subnet_aue)
