# =============================================================================
# VARIABLES.TF - CONNECTIVITY ORCHESTRATOR
# =============================================================================
# NOTE: The following values come from REMOTE STATES (not variables):
# - tenant_id           → foundation.tfstate
# - primary_location    → foundation.tfstate  
# - secondary_location  → foundation.tfstate
# - log_analytics_id    → management.tfstate
# =============================================================================

# -----------------------------------------------------------------------------
# SUBSCRIPTION (Required - from terraform.tfvars with placeholder)
# -----------------------------------------------------------------------------
variable "connectivity_subscription_id" {
  description = "Subscription ID for Connectivity (Hub networks)"
  type        = string
}
...
variable "environment" {
  description = "Environment (Production, NonProduction, Sandbox)"
  type        = string
  default     = "Production"

  validation {
    condition     = contains(["Production", "NonProduction", "Sandbox"], var.environment)
    error_message = "Environment must be Production, NonProduction or Sandbox."
  }
}
...
variable "resource_group_name_network_aue" {
  description = "Network Resource Group name for Australia East"
  type        = string
  default     = "rg-connectivity-network-aue-001"
}

variable "resource_group_name_network_ause" {
  description = "Network Resource Group name for Australia Southeast"
  type        = string
  default     = "rg-connectivity-network-ause-001"
}

variable "resource_group_name_dns_aue" {
  description = "DNS Resource Group name for Australia East"
  type        = string
  default     = "rg-connectivity-dns-aue-001"
}

variable "resource_group_name_dns_ause" {
  description = "DNS Resource Group name for Australia Southeast"
  type        = string
...
variable "hub_vnet_address_space_aue" {
  description = "Address space for Hub VNet Australia East"
  type        = list(string)
  default     = ["10.0.0.0/22"]
}

variable "hub_subnets_aue" {
  description = "Subnet configuration for Hub Australia East"
  type = object({
...
variable "hub_vnet_address_space_ause" {
  description = "Address space for Hub VNet Australia Southeast"
  type        = list(string)
  default     = ["10.1.0.0/22"]
}

variable "hub_subnets_ause" {
  description = "Subnet configuration for Hub Australia Southeast"
  type = object({
    gateway_subnet               = string
    firewall_subnet              = string
    firewall_management_subnet   = string
...
variable "routing_address_space" {
  description = "Address spaces to route through hub (RFC1918 + spokes)"
  type        = list(string)
...
variable "vpn_connections" {
  description = "VPN Site-to-Site connections configuration"
  type = map(object({
    name                = string
...
variable "expressroute_gateway_sku" {
  ...
  validation {
    ...
    error_message = "Must be a valid ExpressRoute Gateway SKU."
  }
}
...
variable "firewall_threat_intel_mode" {
  ...
  validation {
    ...
    error_message = "Must be Alert, Deny or Off."
  }
}
...
variable "bastion_scale_units" {
  description = "Bastion scale units (2-50)"
  type        = number
  default     = 2
...
variable "dns_forward_zones" {
  description = "DNS zones to forward to on-premises"
  type = map(object({
    domain_name              = string
...
variable "enable_ddos_protection" {
  description = "Enable DDoS Protection Plan (disabled when using Cloudflare)"
  type        = bool
  default     = false
}
...
variable "application_gateway_private_ip_address" {
  description = "Private IP address for Application Gateway (optional)"
  type        = string
  default     = null
}
