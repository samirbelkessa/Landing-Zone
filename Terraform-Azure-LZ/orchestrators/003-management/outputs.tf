...
output "la_linked_storage_ids" {
  description = "IDs of Log Analytics linked storage accounts"
  value = {
    custom_logs = try(azurerm_log_analytics_linked_storage_account.custom_logs[0].id, null)
    query       = try(azurerm_log_analytics_linked_storage_account.query[0].id, null)
    alerts      = try(azurerm_log_analytics_linked_storage_account.alerts[0].id, null)
  }
}