...
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
# RESOURCE GROUPS - Multi-RG Architecture
# -----------------------------------------------------------------------------
output "resource_group_ids" {
  description = "IDs des Resource Groups créés par fonction"
  value = {
    network = {
      australiaeast      = azurerm_resource_group.network_aue.id
      australiasoutheast = azurerm_resource_group.network_ause.id
    }
    dns = {
      australiaeast      = azurerm_resource_group.dns_aue.id
      australiasoutheast = azurerm_resource_group.dns_ause.id
    }
  }
}

output "resource_group_names" {
  description = "Noms des Resource Groups créés par fonction"
  value = {
    network = {
      australiaeast      = azurerm_resource_group.network_aue.name
      australiasoutheast = azurerm_resource_group.network_ause.name
    }
    dns = {
      australiaeast      = azurerm_resource_group.dns_aue.name
      australiasoutheast = azurerm_resource_group.dns_ause.name
    }
  }
}
# Legacy outputs for backward compatibility
output "resource_group_id_primary" {
  description = "ID du Resource Group Network principal (legacy)"
  value       = azurerm_resource_group.network_aue.id
}

output "resource_group_id_dr" {
  description = "ID du Resource Group Network DR (legacy)"
  value       = azurerm_resource_group.network_ause.id
}
...
output "spoke_peering_config" {
  ...
  value = {
    australiaeast = {
      ...
      resource_group_name = azurerm_resource_group.network_aue.name  # ← Updated
      ...
      dns_server_ip       = try(module.alz_connectivity.dns_server_ip_addresses["australiaeast"], null)
    }
    australiasoutheast = {
      ...
      resource_group_name = azurerm_resource_group.network_ause.name  # ← Updated
      ...
      dns_server_ip       = try(module.alz_connectivity.dns_server_ip_addresses["australiasoutheast"], null)
    }
  }
}
